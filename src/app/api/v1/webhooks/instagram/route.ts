...
export async function POST(req: NextRequest) {
  const rl = await checkRateLimit(`webhook:instagram:${req.headers.get("x-forwarded-for") ?? "unknown"}`, 300, 60);
  if (!rl.allowed) return NextResponse.json({ error: "Rate limit exceeded" }, { status: 429 });

  const rawBody = await req.text();
  const result = await processInboundWebhook("INSTAGRAM", rawBody, req.headers.get("x-hub-signature-256"));
